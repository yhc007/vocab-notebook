...
- **`main.rs`** — axum server, `AppState { db, extractor, oauth, key }`, route wiring. The app routes (`/`, `/entries`, `/words`, `/words/known`, `/sentences`, `/review`, `/export/*`) sit behind the `auth::require_auth` gate; `/auth/*` are public. `/export/words.csv|.tsv` and `/export/sentences.csv` stream downloads (RFC4180 CSV via `csv_field`, Anki TSV with `#separator`/`#columns` directives), honoring `?category=`. `create_entry` is the core flow. Server-rendered pages share `page()`/`nav()`/`category_filter()` helpers + a `CSS` const. `/review` is a client-side flashcard deck: the server emits `review_words()` (all words minus `known_words`) as JSON in a `<script id="deck">`, and `REVIEW_JS` runs the card loop, calling `/words/known` via fetch on "알아요".
